...
import com.rainyday.saveableapp.platform.AppInfo
import com.rainyday.saveableapp.platform.isDynamicColorSupported
import com.rainyday.saveableapp.platform.rememberBackupFileIO
...
fun SettingsScreen(onBack: () -> Unit) {
    ...
    val backupFileIO = rememberBackupFileIO()
    ...
    fun onExportClick() {
        viewModel.exportData { content ->
            if (content == null) {
                scope.launch { snackbarHostState.showSnackbar("Export failed") }
            } else {
                backupFileIO.exportJson("todo-backup.json", content) { success ->
                    scope.launch { snackbarHostState.showSnackbar(if (success) "Backup saved" else "Export failed") }
                }
            }
        }
    }
    ...
    fun onImportClick() {
        backupFileIO.importJson { content ->
            if (content == null) {
                scope.launch { snackbarHostState.showSnackbar("Import failed — file may be invalid") }
            } else {
                viewModel.importData(content) { success ->
                    scope.launch {
                        snackbarHostState.showSnackbar(if (success) "Backup restored" else "Import failed — file may be invalid")
                    }
                }
            }
        }
    ...
    Scaffold(
        ...
        Column(
            ...
            ListItem(
                ...
                supportingContent = { Text(AppInfo.version) }
            )
        }
    }
}

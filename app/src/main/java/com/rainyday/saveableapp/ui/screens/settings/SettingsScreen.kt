...
fun SettingsScreen() {
    ...
    val driveSignInLauncher = rememberLauncherForActivityResult(
        ...
        driveAccount = runCatching { task.getResult(ApiException::class.java) }.getOrNull()
        if (driveAccount == null) {
            scope.launch { snackbarHostState.showSnackbar("Google sign-in failed or was cancelled") }
        }
    }
    ...
    Scaffold(
        ...
        Column(
            ...
            Column(modifier = Modifier.padding(horizontal = 16.dp)) {
                Text(
                    text = "Used to send quick-add task text to Groq's openai/gpt-oss-20b model to fill in " +
                        "priority, due date, list, and tags automatically.",
                ...
                OutlinedTextField(
                    ...
                    label = { Text("Groq API key (testing only)") },
                    singleLine = true,
